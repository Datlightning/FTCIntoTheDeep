...
//                        camera.stopCamera();
                    ...
                    case TURN_CLAW:
                        if (intake.slides.getCurrentPosition() < 1250 && !move_next_override) {
                            break;
                        }
                        move_next = false;
                        intake.moveWrist(30);
...
//                    camera.useInsidePick(false);
            ...
            if (dpad_down) {
                ...
                double power = sameSignSqrt((-currentGamepad2.left_stick_y/(position.equals(INTAKE_POSITIONS.TURN_CLAW) ? 3.0 : 1.0)));
                if(!robot_go_kaboom) {
